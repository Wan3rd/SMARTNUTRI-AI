...
import { X, ShieldCheck, Scale, AlertTriangle, FileText, Lock, Activity, Info } from 'lucide-react';
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-2xl max-h-[85vh] bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20 flex flex-col"
                    >
                        {/* Header */}
                        <div className="p-6 sm:p-8 border-b border-slate-100 dark:border-white/5 shrink-0 relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between relative z-10">
                                <div>
                                    <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Terms of Service</h2>
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
                                        <Info size={12} className="text-emerald-500" /> Last Updated: May 2026
                                    </p>
                                </div>
                                <button
                                    onClick={onClose}
                                    className="p-2 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 hover:text-slate-900 dark:hover:text-white transition-all"
                                >
                                    <X size={20} />
                                </button>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 custom-scrollbar">
                            ...
                            <div className="grid gap-6">
                                {sections.map((section, idx) => (
                                    <motion.div
                                        ...
                                        className="flex gap-4 p-5 rounded-3xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 transition-all hover:border-emerald-500/30 group"
                                    >
                                        <div className={cn("h-12 w-12 rounded-2xl shrink-0 flex items-center justify-center transition-transform group-hover:scale-110", section.bg, section.color)}>
                                            <section.icon size={24} />
                                        </div>
                                        <div className="space-y-1">
                                            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">{section.title}</h3>
                            ...
                            <div className="p-6 bg-emerald-500/5 rounded-3xl border border-emerald-500/10">
                                <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold leading-relaxed text-center italic">
                        ...
                        {/* Footer */}
                        <div className="p-6 sm:p-8 bg-slate-50 dark:bg-white/5 border-t border-slate-100 dark:border-white/5 shrink-0 flex justify-end gap-4">
                            <Button
                                variant="outline"
                                onClick={onClose}
                                className="px-8 rounded-2xl font-black uppercase tracking-widest text-[10px]"
                            >
                                Close
                            </Button>
                            <Button
                                onClick={onClose}
                                className="px-10 rounded-2xl bg-emerald-500 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-500/20"
                            >
                                I Understand
                            </Button>
                        </div>
